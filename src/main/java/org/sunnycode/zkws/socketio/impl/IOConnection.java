...
class IOConnection implements IOCallback {
  ...
  /**
   * The first socket to be connected. the socket.io server does not send a connected response to
   * this one.
   */
  ...
  /**
   * The Class HearbeatTimeoutTask. Handles dropping this IOConnection if no heartbeat is received
   * within life time.
   */
  ...
  private class ConnectThread extends Thread {
    ...
    @Override
    public void run() {
      if (IOConnection.this.getState() == STATE_INIT) handshake();
      connectTransport();
    }
    ...
    if (list == null) {
      ...
      synchronized (list) {
        for (IOConnection connection : list) {
          if (connection.register(socket)) return connection;
        }
      }
    }
  ...
  public synchronized boolean register(SocketIO socket) {
    ...
    if (sockets.containsKey(namespace)) return false;
  ...
  /**
   * Disconnect a socket from the IOConnection. Shuts down this IOConnection if no further
   * connections are available for this IOConnection.
   * 
  ...
  private synchronized void connectTransport() {
    if (getState() == STATE_INVALID) return;
   ...
   * @return an {@link IOAcknowledge} instance, may be <code>null</code> if server doesn't request
   *         one.
   */
  private IOAcknowledge remoteAcknowledge(IOMessage message) {
    ...
    if (_id.equals(""))
      return null;
    else if (_id.endsWith("+") == false) _id = _id + "+";
    final String id = _id;
  ...
  private void invalidateTransport() {
    if (transport != null) transport.invalidate();
    transport = null;
  }
  ...
  /**
   * finds the corresponding callback object to an incoming message. Returns a dummy callback if no
   * corresponding callback can be found
   * 
   * @param message the message
  ...
  private IOCallback findCallback(IOMessage message) throws SocketIOException {
    if ("".equals(message.getEndpoint())) return this;
  ...
  public void transportMessage(String text) {
    ...
    switch (message.getType()) {
      ...
      case IOMessage.TYPE_JSON_MESSAGE:
        try {
          ...
          if (data.trim().equals("null") == false)
            obj = mapper.readValue(data, LinkedHashMap.class);
          try {
      ...
      case IOMessage.TYPE_EVENT:
        try {
          Map<String, Object> event = mapper.readValue(message.getData(), LinkedHashMap.class);
          Object[] argsArray;
          if (event.containsKey("args")) {
            List<Object> args = (List<Object>) event.get("args");
            argsArray = new Object[args.size()];
            for (int i = 0; i < args.size(); i++) {
              if (args.get(i) != null) argsArray[i] = args.get(i);
            }
          } else
        ...
        if (data.length == 2) {
          try {
            ...
            if (ack == null)
              ...
              List<Object> array = mapper.readValue(data[1], ArrayList.class);
   ...
   * @param json the json
   */
  public void send(SocketIO socket, IOAcknowledge ack, Map<String, Object> json) {
    try {
      IOMessage message =
          new IOMessage(IOMessage.TYPE_JSON_MESSAGE, socket.getNamespace(),
  ...
  public void emit(SocketIO socket, String event, IOAcknowledge ack, Object... args) {
    try {
      ...
      IOMessage message =
          new IOMessage(IOMessage.TYPE_EVENT, socket.getNamespace(),
              mapper.writeValueAsString(json));
      synthesizeAck(message, ack);
      sendPlain(message.toString());
    } catch (Exception e) {
  ...
  private synchronized void setState(int state) {
    if (getState() != STATE_INVALID) this.state = state;
  }
  ...
  public void onDisconnect() {
    ...
    if (socket != null) socket.getCallback().onDisconnect();
  }

  @Override
  public void onConnect() {
    SocketIO socket = sockets.get("");
    if (socket != null) socket.getCallback().onConnect();
  }
