 ...
 * 
 * Copyright (c) 2012, Enno Boland socket.io-java-client is a implementation of the socket.io
 * protocol in Java.
 * 
 * See LICENSE file for more information
 */
package org.sunnycode.zkws.socketio.impl;
...
public class SocketIO {
  ...
  /**
   * Instantiates a new socket.io connection. The object connects after calling
   * {@link #connect(URL, IOCallback)} or {@link #connect(String, IOCallback)}
   */
  public SocketIO() {
  ...
  /**
   * Instantiates a new socket.io connection. The object connects after calling
   * {@link #connect(IOCallback)}
   * 
   * @param url the url
   * @throws MalformedURLException the malformed url exception
   */
  public SocketIO(final String url) throws MalformedURLException {
    if (url == null) throw new RuntimeException("url may not be null.");
    setAndConnect(new URL(url), null);
  }

  /**
   * Instantiates a new socket.io connection and sets the request headers used while connecting the
   * first time for authorizing. The object connects after calling {@link #connect(IOCallback)}
   * 
   * @param url the url
  ...
  /**
   * Instantiates a new socket.io object and connects to the given url. Do not call any of the
   * connect() methods afterwards.
   * 
   * @param url the url
  ...
  /**
   * connects to supplied host using callback. Do only use this method if you instantiate
   * {@link SocketIO} using {@link #SocketIO()}.
   * 
   * @param url the url
   * @param callback the callback
   */
  ...
  /**
   * connects to an already set host. Do only use this method if you instantiate {@link SocketIO}
   * using {@link #SocketIO(String)} or {@link #SocketIO(URL)}.
   * 
   * @param callback the callback
   */
  ...
  /**
   * Emits an event to the Socket.IO server. If the connection is not established, the call will be
   * buffered and sent as soon as it is possible.
   * 
   ...
   */
  public void send(final Map<String, Object> json) {
   ...
   * @param json the JSON object
   */
  public void send(IOAcknowledge ack, final Map<String, Object> json) {
   ...
   * 
   * This had become useful on some android devices which do not shut down tcp-connections when
   ...
   * 
   * @return true if a connection is established, false if the transport is not connected or
  ...
  /**
   * Returns the headers used while handshaking. These Properties are not necessarily the ones set
   * by {@link #addHeader(String, String)} or {@link #SocketIO(String, Properties)} but the ones
   ...
   * {@link #SocketIO(String, Properties)} or {@link #addHeader(String, String)} instead.
   * 
  ...
  public String getHeader(String key) {
    if (this.headers.contains(key)) return this.headers.getProperty(key);
    return null;
  }
}
